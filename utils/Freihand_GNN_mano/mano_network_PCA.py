...
class YTBHand(nn.Module):
    ...
    """
    def __init__(self, spiral_indices, up_transform,use_pca=False,pca_comps=48, use_mean_shape=False):
        ...
        self.mano_layer = ManoLayer(center_idx=9,flat_hand_mean=False,side='right',mano_root=MANO_dir,\
                        use_pca = use_pca,ncomps=pca_comps)
    ...
    def forward(self, x):
        ...
        if self.use_mean_shape:
            beta = torch.zeros(bs, 10).to(device)
        else:
