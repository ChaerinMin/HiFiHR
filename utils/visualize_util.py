...
def displaydemo(mode_train, obj_output, image_output, epoch, idx, vertices, faces, imgs, j2d_gt, open_2dj, j2d, hm_j2d, nimble_j2d, masks, maskRGBs, render_images,joints,joints_gt, nimble_joints, skin_meshes=None, textures=None, re_sil=None, re_img=None, re_depth=None, gt_depth=None,pc_gt_depth=None, pc_re_depth=None, obj_uv6 = None, opt_j2d = None, opt_img=None, dataset_name = 'FreiHand', writer=None, writer_tag='not-sure', console=None, img_wise_save=False, refhand=None, warphand=None, Ks=None, light_params=None, full_pose=None):
    ...
    if skin_meshes is not None:
        demo_path = os.path.join(obj_output, 'demo{}_{:04d}_{:07d}.obj'.format(evalName, epoch, idx))
    ...
    Ks_path = os.path.join(obj_output, 'demo{}_{:04d}_{:07d}_Ks.npy'.format(evalName, epoch, idx))
    if Ks is not None:
        np.save(Ks_path, Ks[0].detach().cpu().numpy())
    light_param_path = os.path.join(obj_output, 'demo{}_{:04d}_{:07d}_lightparams.pickle'.format(evalName, epoch, idx))
    if light_params is not None:
        light_params = {k: v[0] for k,v in light_params.items()}
        torch.save(light_params, light_param_path)
    full_pose_path = os.path.join(obj_output, 'demo{}_{:04d}_{:07d}_full_pose.pickle'.format(evalName, epoch, idx))
    if full_pose is not None:
        assert full_pose.shape[0] ==1, "only support batch size of 1"
        torch.save(full_pose.squeeze(0).detach().cpu(), full_pose_path)
...
def displadic(mode_train, obj_output, image_output, epoch, idx, examples, outputs, dat_name, op_outputs=None, opt_j2d=None, opt_img=None, writer=None, console=None, writer_tag='not-sure', img_wise_save=False):
    ...
    Ks = examples['Ps'] if 'Ps' in examples else None
    light_params = outputs['light_params'] if 'light_params' in outputs else None
    full_pose = outputs['full_pose'] if 'full_pose' in outputs else None
    ...
    displaydemo(mode_train, obj_output=obj_output, image_output=image_output, epoch=epoch, 
                ...
                img_wise_save=img_wise_save, refhand=refhand, warphand = warphand, Ks=Ks, light_params=light_params, full_pose=full_pose)
